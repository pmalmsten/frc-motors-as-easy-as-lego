 ...
 */
public class VelocityControlledDifferentialDrive extends RobotDriveBase {
    // TODO: Reevaluate.
...
//    public static final double kDefaultQuickStopAlpha = 0.1;
    ...
    private VelocityControlledMotor m_leftMotor;
    private VelocityControlledMotor m_rightMotor;
...
//    private double m_quickStopAccumulator = 0.0;
     ...
     */
    public VelocityControlledDifferentialDrive(VelocityControlledMotor leftMotor, VelocityControlledMotor rightMotor) {
     ...
     *
     * @param xSpeed            The robot's speed along the X axis [-1.0..1.0]. Forward is positive.
     * @param zRotationSpeed    The robot's rotation rate around the Z axis [-1.0..1.0]. Clockwise is
     *                          positive.
     * @param squaredInputs     If set, decreases the input sensitivity at low speeds.
     */
    @SuppressWarnings("ParameterName")
    public void arcadeDrive(double xSpeed, double zRotationSpeed, boolean squaredInputs) {
        if (!m_reported) {
        ...
        zRotationSpeed = limit(zRotationSpeed);
        zRotationSpeed = applyDeadband(zRotationSpeed, m_deadband);

        // Square the inputs (while preserving the sign) to increase fine control
        // while permitting full power.
        if (squaredInputs) {
            xSpeed = Math.copySign(xSpeed * xSpeed, xSpeed);
            zRotationSpeed = Math.copySign(zRotationSpeed * zRotationSpeed, zRotationSpeed);
        }

        double leftMotorVelocityPercentage;
        double rightMotorVelocityPercentage;

        double maxInput = Math.copySign(Math.max(Math.abs(xSpeed), Math.abs(zRotationSpeed)), xSpeed);

        if (xSpeed >= 0.0) {
            // First quadrant, else second quadrant
            if (zRotationSpeed >= 0.0) {
                leftMotorVelocityPercentage = maxInput;
                rightMotorVelocityPercentage = xSpeed - zRotationSpeed;
            } else {
                leftMotorVelocityPercentage = xSpeed + zRotationSpeed;
                rightMotorVelocityPercentage = maxInput;
            }
        } else {
        ...
        m_leftMotor.setDesiredVelocityAsPercentage(limit(leftMotorVelocityPercentage) * m_maxOutput);
        m_rightMotor.setDesiredVelocityAsPercentage(-limit(rightMotorVelocityPercentage) * m_maxOutput);
...
//        }
//
//        m_leftMotor.setDesiredVelocityAsPercentage(leftMotorOutput * m_maxOutput);
//        m_rightMotor.setDesiredVelocityAsPercentage(-rightMotorOutput * m_maxOutput);
//
//        m_safetyHelper.feed();
//    }
    ...
    public void tankDrive(double leftSpeed, double rightSpeed, boolean squaredInputs) {
        ...
        m_leftMotor.setDesiredVelocityAsPercentage(leftSpeed * m_maxOutput);
        m_rightMotor.setDesiredVelocityAsPercentage(-rightSpeed * m_maxOutput);
...
//    }
    ...
    public void stopMotor() {
        m_leftMotor.setDesiredVelocityAsPercentage(0);
        m_rightMotor.setDesiredVelocityAsPercentage(0);
        m_safetyHelper.feed();
    }
